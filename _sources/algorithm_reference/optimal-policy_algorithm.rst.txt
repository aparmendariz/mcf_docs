...
- :math:`(R)` : The reward, which is the maximum cumulative potential outcome.
...
2. Case :math:`(L > 1)` : initialize the reward :math:`(R)` to negative infinity and the tree :math:`(\mathcal{T})` to empty. Loop over all features :math:`(m = 1, 2, \ldots, p_1 + p_2)`:
   ...
   For each feature, consider all possible split points:
     ...
     - If the sum of the rewards from the left and right splits exceeds the current maximum reward :math:`(R)`, update :math:`(R)` and :math:`(\mathcal{T})` to reflect the new best split.
